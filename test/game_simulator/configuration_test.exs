defmodule GameSimulator.ConfigurationTest do
  ...
  test "test configuration uses an isolated temporary log directory" do
    ...
    assert %{enabled: false, shadow_mode: true, decision_model: "deepseek/deepseek-v4-flash", interest_threshold: 4} = Configuration.llm!()
  end
  ...
  test "rejects an invalid token TTL" do
    Application.put_env(:game_simulator, :auth,
      ...
      legacy_users_file: Path.join(System.tmp_dir!(), "game_simulator-users"),
      token_ttl_seconds: 0
    )
